...
import {
  ensurePrismaProvider,
  restorePrismaSchemaIfChanged,
  resolveSqliteFileFromEnvValue,
...
async function prepareDatabase(provider: 'sqlite' | 'postgresql') {
  ...
  if (provider === 'postgresql') {
    ...
    if (hasMigrations) {
      ...
      if (!nonProd) {
        console.error(
          'No migrations found and running in production. Please generate and ship migrations.',
        );
        process.exit(1);
      }
      await runWithRetries(
...
function spawnServerWithWatch(useWatch: boolean) {
  return spawn(
    useWatch ? 'npx' : 'node',
    useWatch ? ['tsx', 'watch', 'src/index.ts'] : ['dist/src/index.js'],
    {
      stdio: 'inherit',
      cwd: backendDir,
      env: { ...process.env },
...
function createStop(
  ...
  return async (code?: number) => {
    ...
    if (provider === 'sqlite') {
      await Promise.all(
        Array.from(sqliteDbCandidates).map((p) => fs.rm(p, { force: true }).catch(() => {})),
      );
    }
...
function attachProcessAndServerHandlers(
