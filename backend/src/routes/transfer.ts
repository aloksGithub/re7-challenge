...
function mapTransferError(err: any): {
  status: number;
  message: string;
  code?: string;
  details?: unknown;
  ...
  if (code === 'INSUFFICIENT_FUNDS' || lower.includes('insufficient funds')) {
    return {
      status: 400,
      message: 'Insufficient funds for gas or value',
  ...
  if (lower.includes('private_key is required')) {
    return {
      status: 500,
      message: 'Server misconfigured: missing signing key',
      code: 'CONFIG_ERROR',
    };
  }
