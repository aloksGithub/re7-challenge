...
describe("POST /transfer (unit)", () => {
	...
	it("returns 400 when missing fields or invalid addresses", async () => {
		await request(app).post("/transfer").send({}).expect(400);
		await request(app).post("/transfer").send({ network: "sepolia" }).expect(400);
		await request(app).post("/transfer").send({ network: "sepolia", to: "0x0" }).expect(400);
	});
	...
	it("calls transferErc20 and returns 202", async () => {
		...
		const res = await request(app).post("/transfer").send(body).expect(202);
